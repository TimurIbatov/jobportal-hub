...
CREATE POLICY "Employers can view applicant resumes" ON public.resumes FOR SELECT USING (
  EXISTS (
    ...
    WHERE a.user_id = resumes.user_id AND v.user_id = auth.uid()
  )
);
...
CREATE POLICY "Users can view own resume files" ON storage.objects FOR SELECT
USING (bucket_id = 'resumes' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Employers can view applicant resume files" ON storage.objects FOR SELECT
USING (bucket_id = 'resumes' AND EXISTS (
  SELECT 1 FROM public.applications a
  JOIN public.vacancies v ON v.id = a.vacancy_id
  WHERE a.user_id = (storage.foldername(name))[1]::uuid AND v.user_id = auth.uid()
));

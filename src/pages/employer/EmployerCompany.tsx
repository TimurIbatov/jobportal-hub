    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div><Label>Название компании</Label><Input defaultValue={profile?.company_name || ''} className="rounded-xl mt-1" /></div>
          <div><Label>Контактное лицо</Label><Input defaultValue={`${profile?.first_name || ''} ${profile?.last_name || ''}`} className="rounded-xl mt-1" /></div>
          <div><Label>Email</Label><Input defaultValue="" disabled className="rounded-xl mt-1" /></div>

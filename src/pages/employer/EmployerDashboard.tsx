...
import { getEmployerVacancies, getApplications } from '@/lib/mock-api';
...
export default function EmployerDashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    setVacancies(getEmployerVacancies(user.id));
    setApplications(getApplications({ employer_user_id: user.id }));
  }, [user]);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div><h1 className="text-3xl font-bold text-foreground mb-2">Панель управления</h1><p className="text-muted-foreground">Добро пожаловать, {user?.company_name || user?.first_name}!</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground mb-1">Новых откликов</p><p className="text-3xl font-bold text-foreground">{applications.filter(a => a.status === 'pending').length}</p></div><TrendingUp className="w-8 h-8 text-success opacity-20" /></div></CardContent></Card>
      </div>

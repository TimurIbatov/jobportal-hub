...
import { getApplications, getResumes } from '@/lib/mock-api';
...
export default function CabinetDashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    setApplications(getApplications({ job_seeker_id: user.id }));
    setResumes(getResumes(user.id));
  }, [user]);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-2">Добро пожаловать, {user?.first_name}!</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground mb-1">На рассмотрении</p><p className="text-3xl font-bold text-foreground">{stats.reviewing}</p></div><Clock className="w-8 h-8 text-warning opacity-20" /></div></CardContent></Card>
        <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground mb-1">Принято</p><p className="text-3xl font-bold text-foreground">{stats.accepted}</p></div><TrendingUp className="w-8 h-8 text-success opacity-20" /></div></CardContent></Card>
      </div>
